...
public class UnbreakingUtils {
	...
	public static boolean tryNegateDurabilityCost(ItemStack pStack, int pLevel, RandomSource pRandom, CallbackInfoReturnable<?> cir) {
		ItemStackDurabilityLossEvent event = new ItemStackDurabilityLossEvent(getBase(pStack), pLevel, pStack, pRandom);
		cir.cancel();
		return MinecraftForge.EVENT_BUS.post(event) || (pRandom.nextFloat() > getUnbreakingChance(event.getValue(), pLevel));
	}
	...
	public static float getBase(ItemStack pStack) {
		return StackUtils.getFloat(pStack, "unbreaking", 0);
	}
	...
	public static int tryReduceMultiDurabilityCost(int amount, ItemStack pStack, int pLevel, RandomSource pRandom) {
		ItemStackMultiDurabilityLossEvent event = new ItemStackMultiDurabilityLossEvent(amount, getBase(pStack), pLevel, pStack, pRandom);
		if(MinecraftForge.EVENT_BUS.post(event)) return 0;
		int base = (int) (amount * getUnbreakingChance(event.getValue(), pLevel));
		return base + (((amount-base)>=pRandom.nextFloat()) ? 1 : 0);
	}
}
